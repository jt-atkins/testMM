      ...
      * Check for numbers in the first name                            *
       CHECK-FIRST.
           IF FUNCTION UPPER-CASE (CA-POSTCODE(1:2)) = 'GB'
               *> Prefix 44 to CA-PHONE-MOB for GB
               CONTINUE
           ELSE IF FUNCTION UPPER-CASE (CA-POSTCODE(1:2)) = 'US'
               *> Prefix 01 to CA-PHONE-MOB for US
               CONTINUE
           ELSE IF FUNCTION UPPER-CASE (CA-POSTCODE(1:2)) = 'UK'
               *> Additional logic for UK
               CONTINUE
           ELSE IF FUNCTION UPPER-CASE (CA-POSTCODE(1:2)) = 'DN'
               *> Additional logic for DN
               CONTINUE
           ELSE
               MOVE 'POSTCODE IS INVALID' TO EM-VARIABLE
      *        PERFORM WRITE-ERROR-MESSAGE
               EXEC CICS ABEND ABCODE('PCER') NODUMP END-EXEC
           END-IF.
